...
R"w2c_template(          table.data[x].func_type == func_types[ft]) || \
)w2c_template"
...
R"w2c_template(  uint32_t func_type_index;
)w2c_template"
...
R"w2c_template(                                      const u32* func_types) {
)w2c_template"
R"w2c_template(  if (UNLIKELY(src_addr + (uint64_t)n > src_size))
)w2c_template"
R"w2c_template(    TRAP(OOB);
...
R"w2c_template(        func_types[src_expr->func_type_index], src_expr->func,
)w2c_template"
